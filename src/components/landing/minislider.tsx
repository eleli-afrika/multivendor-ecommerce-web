...
const CardSlider: React.FC<AdFormProps> = ({ Ads }) => {
  ...
  return (
    <div className="wrapper mt-5 overflow-hidden " ref={wrapperRef}>
      <div className="px-4  w-full overflow-hidden bg-white price rounded-b-[8px]">
        <i id="left" className="fa-solid fa-angle-left text-center">
          <FaChevronLeft />
        </i>
        <ul
        ...
        >
          {Ads.map((item: any) => (
            <li
              ...
              key={item?.producttid}
              onClick={() => navigate(`ad_info/${item?.producttid}`)}
            >
              <div className="img">
                <img
              ...
              <p className='className="text-black-main text-[18px] font-bold mt-2 px-[6px]'>
                Ksh {formatPriceWithCommas(item?.productprice)}
              </p>
            </li>
          ))}
        ...
        <i
          id="right"
          className="fa-solid fa-angle-right item-center text-center"
        >
          <FaChevronRight />
        </i>
      </div>
