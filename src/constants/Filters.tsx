...
const Filters = ({ Ads }: any) => {
  ...
  const handlePriceRangeClick = (range: string) => {
    ...
    const filteredAds = Ads?.filter(
      (ad: any) =>
        ad.product_data.productprice >= min &&
        ad.product_data.productprice <= max
    );
  ...
  const sortAds = (sortBy: string) => {
    ...
    switch (sortBy) {
      case "minPrice":
        sortedAds.sort(
          (a, b) => a.product_data.productprice - b.product_data.productprice
        );
        break;
      case "maxPrice":
        sortedAds.sort(
          (a, b) => b.product_data.productprice - a.product_data.productprice
        );
        break;
      case "brand":
        sortedAds.sort((a, b) =>
          a.product_data.brand.localeCompare(b.product_data.brand)
        );
        break;
  ...
  const categories = Array.from(
    new Set(Ads?.map((ad: any) => ad.product_data.category))
  );
